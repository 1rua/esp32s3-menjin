/**
 * ESP32-S3 Mech Master (Grey Wind Patched Edition)
 * Fixed: Removed blocking MQTT loop in setup() to prevent lockout during network failure.
...
void setup() {
  ...
  // 尽量先更新一次天气，但如果不成功也不要卡死
  if(WiFi.status() == WL_CONNECTED) {
      updateWeather();
  }
  safeDelay(500); // 稍微减少等待时间
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (client.connect(mqtt_uid)) {
      ...
      Serial.println(" (Will retry in background loop)");
      // 不要在这里死循环！
    }
  } else {
      Serial.println(" WiFi not ready. Skipping MQTT initial connect.");
  }
...
void enterEnrollMode() {
  ...
  Serial.println("=== ENROLLMENT FINISHED. RETURN TO NORMAL MODE ===");
  playLocalFile("/boot.mp3"); 
}
...
void processArriveHome(String method) {
  ...
  time_t now;
  time(&now); 
  // 如果没有成功获取过天气，sunsetTime可能是0，这里加个判断防止误触发
