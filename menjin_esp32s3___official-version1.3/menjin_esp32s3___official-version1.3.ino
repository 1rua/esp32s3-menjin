/**
 * ESP32-S3 Mech Master (Version 1.1 + OTA)
 * * Base: Official Version 1.1
 * * Added: ArduinoOTA (Wireless Firmware Update)
 * * Kept: Audio, 3 Servos, Original Logic
...
void safeDelay(unsigned long ms) {
  ...
  while(millis() - start < ms) {
    audio.loop(); 
    ArduinoOTA.handle(); // [新增] 延时期间也要维持OTA
