...
- **新增 Web 管理门户**：可通过浏览器执行开门、写入 NFC、配置网络、管理长期 PIN、生成/撤销临时 PIN、修改管理员密码。
- **新增管理员认证**：Web 门户统一走 Basic Auth，默认管理员账号为 `admin`，默认密码为 `esp32s3-menjin`。
- **新增 PIN 管理模块**：长期 PIN / 临时 PIN 独立管理，存储在 NVS 中；PIN 与管理员密码均以**带随机盐的 SHA-256 哈希**保存。
